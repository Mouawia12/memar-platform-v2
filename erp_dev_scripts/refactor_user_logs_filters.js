...
if (ix !== -1 && endIx !== -1) {
  ...
  switchTab(tab) {
    ...
    this.state.filters.type = 'all'; // Reset specific type filter on tab switch
    this.render();
  },
  ...
  applyFilters(usersList) {
    return usersList.filter(u => {
        ...
        // 5. Activity Filter (Mock: recent means has logged in, inactive means never)
  ...
  render() {
    ...
    // Split users conceptually based on tabs
          ...
          <tbody>
            \${paginatedList.map(u => {
              ...
              <tr style="border-bottom:1px solid var(--border); \${isSuspended ? 'background:#fef2f2; opacity:0.8;' : isDeleted ? 'background:#f8fafc; opacity:0.5;' : ''} transition:background 0.2s">
                ...
                <td style="padding:12px; text-align:center;"><span class="badge \${isSuspended ? 'badge-red' : isDeleted ? 'badge-gray' : 'badge-green'}">\${isDeleted ? 'محذوف 🗑️' : isSuspended ? 'موقوف ❄️' : 'نشط ✅'}</span></td>
  ...
  deleteUser(id) {
     if (!confirm('هل أنت متأكد من حذف الحساب منطقياً (Soft Delete)؟')) return;
     const user = window.DB_TABLES.users.find(u => u.id === id);
     if (!user) return;
     user.status = 'deleted';
  ...
  restoreUser(id) {
    ...
    console.log('Successfully injected UserLogs with Advanced Filters!');
} else {
