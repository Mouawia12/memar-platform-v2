...
import { useDeleteLead, useLeads, useMoveLead, useSetTemperature } from '../hooks/useCrm';
...
export function CrmPage() {
  ...
  const handleSetTemp = (l: Lead, temperature: Temperature) => setTemp.mutate({ id: l.id, temperature });
  ...
  return (
    <div>
      ...
      {data && (
        <CrmBoard
          leads={leads} onEdit={openEdit} onDelete={handleDelete} onMove={handleMove}
          onAddTask={handleAddTask} onHistory={setHistory} onSetTemp={handleSetTemp}
        />
      )}
      ...
      {history && <LeadHistoryModal lead={history} onClose={() => setHistory(null)} />}
    </div>
  );
}
