...
def update_file():
    with open('erp/pricing2.js', 'r', encoding='utf-8') as f:
        content = f.read()

    # 1. Replace PricingDB2
    new_db = """const PricingDB2 = {
  ...
  calcTotal() {
    ...
    # Replacing DB, State, Calc
    content = re.sub(r'const PricingDB2 = \{.*?\n\};\n', new_db + '\n', content, flags=re.DOTALL)
    content = re.sub(r'const PricingState2 = \{.*?\n\};\n', new_state + '\n', content, flags=re.DOTALL)
    content = re.sub(r'const PriceCalc2 = \{.*?\n\};\n', new_calc + '\n', content, flags=re.DOTALL)

    # 2. Update renderCategoryTabs
    old_tabs = """renderCategoryTabs() {
    return `
    ...
    # 3. Update renderServicesSection group labels
    ...
    # Update manual pricing UI
    old_svc_ui = """${PricingState2.adminMode ? `
    ...
    new_svc_ui = """${PricingState2.adminMode ? `
    ...
    # 4. Update Summary
    old_docs_filter = """const reqDocs = PricingDB2.documentsMaster.filter(d => {
    ...
    new_docs_filter = """const reqDocs = PricingDB2.documentsMaster.filter(d => {
      if (PricingState2.category === 'residential') {
    ...
    # Update manual pricing total formatting in Summary
    content = content.replace("${this.fmt(l.amount)}", "${l.isManual ? 'تسعير يدوي' : this.fmt(l.amount)}")
    ...
    # Fix pdf export to show manual pricing
    ...
    with open('erp/pricing2.js', 'w', encoding='utf-8') as f:
        f.write(content)
